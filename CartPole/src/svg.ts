...
export class Circle extends SvgItem {
  public constructor(classNames: Array<string>, x: number, y: number) {
    super("circle");
    this._element.setAttribute("r", "0.01");
...
export class Rect extends SvgItem {
  ...
  public constructor(classNames: Array<string>, cx: number, cy: number) {
    ...
    this.addClassNames(classNames);
    this._element.setAttribute("width", xFromSimulatorToScreen(this.width).toString());
    this._element.setAttribute("height", xFromSimulatorToScreen(this.height).toString());
  }

  public updatePosition(cx: number, cy: number) {
    this.setPosition(cx, cy);
  }

  private setPosition(cx: number, cy: number) {
    this._element.setAttribute("x", xFromSimulatorToScreen(cx - 0.5 * this.width).toString());
    this._element.setAttribute("y", yFromSimulatorToScreen(cy + 0.5 * this.height).toString());
  }
}
